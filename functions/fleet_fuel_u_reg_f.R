#' fleet_fuel_u_reg_f
#' Function: DEPRECIATED. Calculates on-road fuel use of LDVs by region.
#' @import tidyr
#' @importFrom reshape2 acast
#' @export
fleet_fuel_u_reg_f <- function (first_yr = NA,last_yr = NA){
  attribute_f("fleet_fuel_u_reg_f")
  #Inputs files
  ...
  #fleet_vkt
  fleet_vkt_reg_f_res <- do.call(fun_res_f,list(fun_name="fleet_vkt_reg_f"))
  fleet_vint_vkt <- fleet_vkt_reg_f_res[["fleet_vint_vkt"]]
  #Creation output files
  #fuel_use is the data.frame with the fuel use per year, scenario and fuel
  fleet_vint_fuel_use <- NULL
  for (reg in unique(fleet_vint_vkt$Region)){
    for (size in c("Car", "Light truck")) {
      ...
      for (techno in unique(vh_techno$Own)) {
        ...
        stock_vkt <- subset(fleet_vint_vkt,Region==reg & Size==size & Technology == techno & Year%in%c(first_yr:last_yr))
        ...
        for (fuel_type in fuel_l) {
          ...
          tmp_fuel_use_dt <- as.data.frame(matrix_fuel_use) %>% 
            ...
            cbind(.,Region=reg,Technology=techno,Size=size,Fuel=fuel_type,Unit=fuel_unit,stringsAsFactors = FALSE)
  ...
  #Get electricity use by region
  reg_elec_use <- fleet_vint_fuel_use %>%
    subset(.,Fuel=="Electricity") %>%
    aggregate(formula= Value ~ Region + Year + Unit,data=., FUN=sum)
  
  results<-list(fleet_vint_fuel_use=fleet_vint_fuel_use,
                reg_elec_use=reg_elec_use)
  return(results)
}
