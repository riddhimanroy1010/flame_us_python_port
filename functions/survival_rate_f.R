...
survival_rate_f<-function(age,size,survival_rate_mdl=NA,cumulative_rate="n",scrappage_rate="n"){
  attribute_f(fun_name="survival_rate_f")
  if (survival_rate_mdl%in%c(1,2,3)){
    #Define survival_rates_dt
    #Inputs
survival_rates  <- get_input_f(input_name = 'survival_rates')
    scrap_rate_src <- switch(as.character(survival_rate_mdl),
                             "1"="own",
                             "2"="VISION",
                             "3"="TEDB")
